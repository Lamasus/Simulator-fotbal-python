...
def match_poisson_mc(team1,team2):
    ...
    res_x = collections.Counter({key: counter_x[key] / 100 for key in counter_x}) #calculca pb pt fiecare scor
    res_y = collections.Counter({key: counter_y[key] / 100 for key in counter_y})
    res_x = collections.OrderedDict(sorted(res_x.items())) #sortam dictionarele in functie de chei
    res_y = collections.OrderedDict(sorted(res_y.items()))
    #print(counter_x)
    #print(counter_y)
    print(res_x)
    print(res_y)
    ...
    for i in res_x.keys():
        for j in res_y.keys():
            ...
            if i > j:
                ...
                away_win += rez
    print('Cotele pentru victorie gazde,egal si victorie oaspeti %.2f---%.2f----%.2f' % (1/home_win, 1/draw, 1/away_win))
    scores = collections.Counter({key: 1 / scores[key] for key in scores})
    scores = dict(sorted(scores.items(), key=operator.itemgetter(1)))
    print('Cotele pentru Scor corect(crescator)')
    print(scores)
    ...
    for i in res_x.keys():
        for j in res_y.keys():
            ...
            if i > j:
                ...
                away_win += rez
    print('Cotele pentru victorie gazde,egal si victorie oaspeti %.2f---%.2f----%.2f' % (
    1 / home_win, 1 / draw, 1 / away_win))
    scores = collections.Counter({key: 1 / scores[key] for key in scores})
    scores = dict(sorted(scores.items(), key=operator.itemgetter(1)))
    print('Cotele pentru Scor corect(crescator)')
    print(scores)
